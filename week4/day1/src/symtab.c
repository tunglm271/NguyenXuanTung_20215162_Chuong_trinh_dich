...
Type* duplicateType(Type* type) {
  // TODO
  Type* clonedType = (Type*) malloc(sizeof(Type));
  clonedType->typeClass = type->typeClass;
  if (type->typeClass == TP_ARRAY) {
    clonedType->arraySize = type->arraySize;
    clonedType->elementType = duplicateType(type->elementType);
  }
  return clonedType;
}
